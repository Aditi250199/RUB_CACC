#  controller simulation Test script
...
from final_CACC_controller_setup import extended_CACC
...
class EgoVehicle:
    ...
    def update_state(self, a, omega, dt):
        # Force to float, avoid any unexpected issues with numpy array accidentally
        self.vel.linear = float(self.vel.linear + a * dt)
        self.theta = float(self.theta + omega * dt)
        self.x = float(self.x + self.vel.linear * np.cos(self.theta) * dt)
        self.y = float(self.y + self.vel.linear * np.sin(self.theta) * dt)
...
controller = extended_CACC(k1=1.0, k2= 1.0, headway=0.2, time_gap=0.2)
...
for idx, row in lead_data.iterrows():
    ...
    #relative position computation of ego wrt lead
    dx = lead.x - ego.x
    dy = lead.y - ego.y
    dist = np.hypot(dx, dy)  #Euclidean distance between vehicles
    azimuth = np.arctan2(dy, dx) - ego.theta  #angle between the lead and ego vehicles
    lead.pos.dist = dist
